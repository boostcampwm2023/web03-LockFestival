...
export class ThemeRepository extends Repository<Theme> {
  ...
  async getThemeDetailsById(themeId: number): Promise<ThemeBranchDeatailsResponseDto> {
    const themeDetailsResponseDto: ThemeBranchDeatailsResponseDto = await this.dataSource
      .createQueryBuilder()
      .select([
        'theme.name as name',
        'theme.id as themeId',
