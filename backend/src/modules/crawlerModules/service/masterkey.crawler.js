...
const BASE_URL = 'https://www.master-key.co.kr';
...
const getInfoByData = async ({ shop, date }) => {
...
export const getTimeTableByTheme = async ({ shop: shopId, theme, date }) => {
  ...
  const data = await getInfoByData({ shop, date });
    ...
    if (themeTitle.text().trim() === theme) {
      $(element)
        ...
        .each((index, element) => {
          ...
          if ($(element).hasClass('true')) timeObject['possible'] = true;
          else timeObject['possible'] = false;
